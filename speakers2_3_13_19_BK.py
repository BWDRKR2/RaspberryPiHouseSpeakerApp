...
def status():
	...
	session.modified = True
    	...
    	uri = 'http://10.68.54.18:8000/room_status/'
    	r = requests.get(uri)
    	b = r.json()
...
def on(room_on):
    if current_user.is_authenticated:
	session.modified = True
    	uri = 'http://10.68.54.18:8000/room_on/' + room_on
    	r = requests.get(uri)
    	b = r.json()
    	status=r.status_code
    	return render_template('index1.html', test = b )
...
def off(room_off):
    if current_user.is_authenticated:
	session.modified = True
    	uri = 'http://10.68.54.18:8000/room_off/' + room_off
    	r = requests.get(uri)
    	b = r.json()
    	status=r.status_code
    	return render_template('index1.html', test = b)
...
def validate():
	...
	uri = 'http://10.68.54.18:8000/validate_user/' + username + "/" + password
    	r = requests.get(uri)
    	b = r.json()
    	status=r.status_code
        user_id = b['id']
